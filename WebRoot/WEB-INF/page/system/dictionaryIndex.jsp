	...
	<HEAD>
     ...
     function saveDict(){
	      ...
	      for (i=1;i<tbl.rows.length;i++){   
	   	  ...
	   	  for(k=1;k<=tbl.rows.length-2;k++)//k=1;k<=3;k++
		  {
	 	  	for(m=k+1;m<=tbl.rows.length-1;m++)//m=2;m<=4;m++
	 	  	{     
...
function insertRows(){ 
	//获取表格对象
	var tb1 = $("#dictTbl");
	var tempRow = $("#dictTbl tr").size();//获取dictTbl表格的行数
				...
				<tr>
					...
					<%-- <select name="keyword" class="bg" style="width:180px" onchange="changetype()">
						<s:if test="#request.list!=null && #request.list.size()>0">
							<option value="jerrynew">--请选择--</option>
							<s:iterator value="#request.list">
								<option value="<s:property value="keyword"/>"><s:property value="keyword"/></option>
							</s:iterator>
						</s:if>
					</select> --%>
